...
func GeneratePolicy(data map[int]string) {
	for i := 0; i < len(data); i++ {
		fmt.Println(i, data[i])
		var g, s, d, r, o, t string
		rows := DB.QueryRow("select _Groups,_SubjectMark,_Diseases,_Researcher,_GatherTime FROM base_info where _CaseNumber='" + data[i] + "'")
		rows.Scan(&g, &s, &d, &r, &t)
		result := make(map[int]string)
		result = queryDB("select insti_name from insti_coop where _CaseNumber='" + data[i] + "'")
		...
		policy := abac.AutoPolicyCreate(abac.Date2DB(attr), data[i], t)
		var str string
		SQLString := "select * from policy where policy_id='" + data[i] + "'"
		err := DB.QueryRow(SQLString).Scan(&str)
		if err == sql.ErrNoRows { //没有结果
			SQLString3 := "insert into policy(policy_id,policy_data)values(?,?)"
			_, err := DB.Exec(SQLString3, data[i], policy)
			if err != nil {
				fmt.Println("err")
			}
		}
